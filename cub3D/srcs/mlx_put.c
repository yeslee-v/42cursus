...
/*   Updated: 2021/04/12 17:39:59 by yeslee           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_put_step_side_value(t_all *all)
{
	...
	{
		all->info.step_x = -1;
		all->info.side_x = (all->info.pos_x - all->info.map_x) *
			all->info.delta_x;
	}
	else
	{
		all->info.step_x = 1;
		all->info.side_x = (all->info.map_x + 1.0 - all->info.pos_x) *
			all->info.delta_x;
	}
	if (all->info.ray_y < 0)
	{
		all->info.step_y = -1;
		all->info.side_y = (all->info.pos_y - all->info.map_y) *
			all->info.delta_y;
	}
	else
	{
		all->info.step_y = 1;
		all->info.side_y = (all->info.map_y + 1.0 - all->info.pos_y) *
			all->info.delta_y;
	}
}
...
void	ft_paint_texture(t_all *all, int x)
{
	...
	all->info.tex_pos = (all->info.draw_s - all->game.r.height / 2.0 +
			all->info.line_h / 2.0) * all->info.step;
	while (y < all->info.draw_e)
	{
		all->info.tex_y = (int)all->info.tex_pos & (all->info.tex_h - 1);
		all->info.tex_pos += all->info.step;
		all->info.color = all->info.tex[all->info.tex_n]
			[all->info.tex_h * all->info.tex_y + all->info.tex_x];
		if (all->info.side == 1)
